/*
* @Author: apple
* @Date:   2017-09-17 18:16:20
* @Last Modified by:   apple
* @Last Modified time: 2017-09-21 17:32:09
*/
$(function(){
    var k = $(window).height();
    var flag = false;
    // 点击一下next 往下滚动一屏
    $(".next").click(function () {
        $.fn.fullpage.moveSectionDown();  // fullpage 的方法
    });
	$('#fullpage').fullpage({    // fullpage  方法里面接受json对象形式
        navigation: true,         // 是否显示项目导航
        loopBottom: true,         //loopBottom  默认值：false，定义屏幕滚动到最后一个后，是否循环滚动到第一个。
        scrollingSpeed: 1400,        // scrollingSpeed  默认值：700，每个屏幕滚动动画执行的时间，时间的单位为毫秒（ms）。


        // 回调函数组
        // 回调函数滚动到第二屏后的回调函数，接收 anchorLink 和 index 两个参数，anchorLink 是锚链接的名称，index 是序号，从1开始计算
        afterLoad: function (anchorLink, index) {
            if(index == 2){
                $(".next").fadeOut();  // 往第二屏滚动的时候   next继续往下先消失
                $(".search").show().animate({"right": 370}, 1500, function() {
                  // 方块走出来  沙发二字显示
                    $(".search-words").animate({"opacity": 1}, 500, function () {
                        $(".search").hide();
                        // 图片 往右上角， 缩小
                        $(".search-02-1").show().animate({"height": 30, "right": 250, "bottom": 452}, 1000);
                        // 同时 沙发图片 变大
                        $(".goods-1").show().animate({"height": 218}, 1000);
                        // 同时 白色文字渐渐的显示出来
                        $(".words-02").animate({"opacity": 1}, 500,function () {
                            $(".next").fadeIn();  // 动画执行结束  继续往下显示出来
                        });
                    });
              });
            }
        },

        // 刚开始滚动屏幕就开始触发的回调函数
        // 滚动前的回调函数，接收 index、nextIndex 和 direction 3个参数：index 是离开的“页面”的序号，从1开始计算；nextIndex
        // 是滚动到的“页面”的序号，从1开始计算；direction 判断往上滚动还是往下滚动，值是 up 或 down。
        onLeave: function (index, nextIndex, direction) {
            $(".next").fadeOut();
            if (index == 2 && nextIndex == 3) {
                // 当第二屏幕往第三屏幕滚动的时候， 沙发显示并且往第三屏幕跑  白色盒子显示
                // 沙发要往第三屏幕走，  走的距离 就是  当前哦屏幕的高度 - main 到底部的高度 - 沙发到main的距离
                // (当前屏幕的高度  - 250 )
                $(".shirt-02").show().animate({"bottom": -(k - 250),"width":207,"left": 260}, 2000, function () {
                    $(".img-01-a").animate({"opacity": 1}, 500, function () {
                        $(".btn-01-a").animate({"opacity": 1}, 500, function () {
                            $(".next").fadeIn();
                        });
                    });
                });
                $(".cover").show();
            }
            // 第3屏幕到第4屏幕过渡
            if (index == 3 && nextIndex == 4) {
                $(".shirt-03").hide();
                $(".t1f").show().animate({"bottom":-((k-250)+50), "left":250}, 2000, function () {
                    $(this).hide();
                    $(".car").animate({"left": "150%"}, 2000, "easeInElastic", function () {
                        $(".note").show();
                        $(".note-img, .words-04-a").animate({"opacity": 1}, 1000, function () {
                            $(".next").fadeIn();
                        });
                    });

                });
            }
            // 第4屏幕到第5屏幕过渡
            if(index == 4, nextIndex == 5){
                // 手出来
                $(".hand-05").animate({"bottom":0}, 2000, function () {
                    // 手出来后  亮鼠标出来
                    $("mouse-05-a").animate({"opacity":0});
                    // 沙发掉下来
                    $(".t1f-05").show().animate({"bottom":70},1000, function () {
                        // 银行单子出来
                        $(".order-05").animate({"bottom":390}, function () {
                            $(".next").fadeIn();
                        });
                    })

                })
            }
            // 第5屏幕到第6屏幕过渡
            if(index == 5, nextIndex == 6){
                $(".t1f-05").animate({"bottom":-(k - 500),"left": "40%", "width": 65},1500, function () {
                    $(".t1f-05").hide();
                });
                $(".box-06").animate({"left": "38%"}, 1500, function () {
                    $(".box-06").animate({"bottom": 40}, 500, function () {
                        $(".box-06").hide();
                        // 行走的过程中  背景移动
                        $(".section6").animate({"backgroundPositionX": "95%"}, 4000, function () {
                            // 当背景动画执行完毕  body大小复原
                            $(".body").animate({"height":305,"bottom":116}, 1000, function () {
                                $(".body").animate({"right": 500}, 500, function () {
                                    $(".door-06").animate({"opacity":1}, 500, function () {
                                        $(".girl").show().animate({"right": 390, "height": 280}, 500, function () {
                                            $(".pop-07").animate({"opacity":1}, 500, function () {
                                                $(".next").fadeIn();
                                                $(".pop-07").show();
                                            });
                                        });
                                    })
                                });
                            })
                        });
                        // 标签语   光的速度
                        $(".words-06").show().animate({"left": "30%"});
                        $(".pop-06").show();

                    })
                })
            }
            if(index = 6 && nextIndex == 7){
                setTimeout(function () {
                    $(".next").fadeIn();
                }, 1000);
            }
            /*第八屛的制作*/
            // 大手跟鼠标移动
            $(document).mousemove(function (event) {
                var x = event.pageX - $(".hand-08").width() / 2;  /*获取鼠标在页面x轴的坐标*/
                var y = event.pageY + 10;  /*获取鼠标在页面y轴中的坐标*/
                // 手的top值不能小于（当前屏幕的高度-手图片的高度）（k-449）
                var minY = k - 449;
                // if(y < minY){
                //     $(".hand-08").css({"left": x, "top": minY});
                // }else{
                //     // 把鼠标在页面中的坐标位置  给大手
                //     $(".hand-08").css({"left": x, "top": y});
                // }
                // 更简洁的做法
                if(y < minY){
                    y = minY
                }
                $(".hand-08").css({"left": x, "top": y});
            });
            // 当我们点击  再来一次的时候  分两步执行
            $(".again-08").click(function (event) {
                // 1.返回到第一屏幕
                $.fn.fullpage.moveTo(1);  // fullpage 方法   滚动到第几屏幕
                // 2.所有的动画都复原  就是原来的样子
                // 核心原理就是让我们的图片（做动画的元素 清除 行内样式就可以）
                // 所有动画的div盒子添加move 类名
                $("img,move").attr("style", " ");  // attr 属性的意思
            });


        }
    });
});
	//var k = $(window).height(); // 当前屏幕的高度
	//var flag = false;  // 控制动画
   /* $('#fullpage').fullpage({ // fullpage  方法里面接受json对象形式

    	// 是否显示项目导航
    	navigation: true,
    	// navigationPosition: "left",
    	// loopBottom: true,
    	// 滚动速度，单位为毫秒
    	scrollingSpeed: 1200,

    	// 回调函数滚动到第二屏后的回调函数，接收 anchorLink 和 index 两个参数，anchorLink 是锚链接的名称，index 是序号，从1开始计算
    	afterLoad: function(anchorLink, index) {
    		if(index == 2 && flag == false) {
    			$(".search").show().animate({"right": 370}, 1500, function() {
    				// 方块走进来，沙发2个字显示
    				$(".search-wrods").animate({"opacity": 1},500, function() {
    					$(".search").hide();
    					// 图片 往右上角， 缩小
    					$(".search-02-1").show().animate({"height": 30, "right": 250, "bottom": 452}, 1000, function() {
    						 flag = true;  // 如果flag 等于true 说明 所有动画执行完毕
    					});
    					// 同时 沙发图片 变大
    					$(".goods-02").show().animate({"height": 218}, 1000)
    					// 同时 白色文字渐渐的显示出来
    					$(".words-02").animate({"opacity": 1},500)
    				});
    			});
    		}
    	},

    	// 刚开始滚动屏幕就触发的回调函数 onLeave
    	// 滚动前的回调函数，接收 index、nextIndex 和 direction 3个参数：index 是离开的“页面”的序号，从1开始计算；nextIndex 是滚动到的“页面”的序号，从1开始计算；direction 判断往上滚动还是往下滚动，值是 up 或 down。
    	onLeave: function(index, nextIndex, direction) {
    		if(index == 2 && nextIndex == 3 && flag== true) {
    			// 当第二屏幕往第三屏幕滚动的时候， 沙发显示并且往第三屏幕跑  白色盒子显示
    			// 沙发要往第三屏幕走，  走的距离 就是  当前哦屏幕的高度 - main 到底部的高度 - 沙发到main的距离    (当前屏幕的高度  - 250 )
    			$(".shirt-02").show().animate({"bottom": -(k - 250), "width": 207 , "left": 260}, 2000, function() {
    				$(".img-01-a").animate({"opacity": 1}, 500, function() {
    					$(".btn-01-a").animate({"opacity": 1}, 500);
    				})
    			});
    			$(".cover").show();
    		}
    		// 第3屏幕到第4屏幕过渡
    		if(index == 3 && nextIndex == 4 ) {
    			$(".shirt-02").hide();
    			 // 沙发的距离   当前屏幕的高度  - 250    + 第三屏幕的 50距离
    			$(".t1f").show().animate({"bottom" : -((k - 250) + 50), "left": 260 }, 2000, function() {
    				 $(this).hide();  // 动画完毕之后，自动隐藏
    				 $(".car-img").show();
    				 // 这购物车就开始往右走
    				 $(".car").animate({"left": "150%"}, 4000);
    			})

    		}
    	},
  
    });  
});*/